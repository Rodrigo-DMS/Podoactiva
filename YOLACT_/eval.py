...
# TEST 
def expand_bbox(bbox, expansion_ratio=0.2, frame_height=None):
    """
    Expand the bounding box vertically by the given ratio.

    Args:
    - bbox (tuple): The bounding box in the format (x1, y1, x2, y2).
    - expansion_ratio (float): Ratio by which to expand the bounding box vertically.
    - frame_height (int, optional): Height of the frame to ensure the expanded bbox doesn't exceed frame boundaries.
    ...
    expand_amount = height * expansion_ratio / 2.0

    # Expand the bbox
    y1_new = max(0, y1 - expand_amount)  # Ensure it doesn't go beyond the top boundary
    y2_new = y2 + expand_amount
    if frame_height:
        y2_new = min(frame_height, y2_new)  # Ensure it doesn't go beyond the bottom boundary
...
class Detections:
    ...
    def add_bbox(self, image_id:int, category_id:int, bbox:list, score:float):
        ...
        # NEW
        print(bbox)
        bbox = expand_bbox(bbox, expansion_ratio=1.5, frame_height=frame_height)
        print(bbox)
        # END NEW
...
def evalimages(net:Yolact, input_folder:str, output_folder:str):
    ...
    for p in Path(input_folder).glob('*'): 
        ...
        print(path + ' -> ' + out_path)
        print(np.round(time.time() - _, 4))
    print('Done.')

# from typing import List
# def evalbatch(net: Yolact, paths: List[str], save_paths: List[str]):
#     # Load images into a list
#     frames = [torch.from_numpy(cv2.imread(path)).cuda().float() for path in paths]
#     batch_frames = torch.stack(frames)
#     print("SHAPE",batch_frames.shape)
#     # Transform and get predictions
#     batch = FastBaseTransform()(batch_frames)
#     preds = net(batch)
#     print(len(preds))
#     print(preds[0]['detection']['mask'])
#     # Process each image in the batch
#     img_numpy = prep_display(preds, frames[0], None, None, undo_transform=False, img_path=save_paths[0])
#     if save_paths[0] is not None:
#             cv2.imwrite(save_paths[0], img_numpy[:, :, (2, 1, 0)])
#     # c = 0
#     # for frame, pred, save_path in zip(frames, preds, save_paths):
#     #     # Assuming preds is a list where each element corresponds to an image in the batch
#     #     img_numpy = prep_display(pred, frame, None, None, undo_transform=False, img_path=save_path, batchIndex = c)
#     #     print(c)
#     #     c += 1
#     #     if save_path is not None:
#     #         cv2.imwrite(save_path, img_numpy[:, :, (2, 1, 0)])
#     print('batch')

# def evalimages(net: Yolact, input_folder: str, output_folder: str, batch_size=32):
#     if not os.path.exists(output_folder):
#         os.mkdir(output_folder)

#     all_paths = [str(p) for p in Path(input_folder).glob('*')]
#     total = len(all_paths)
#     print("N paths", total)
#     print()
...
#         names = ['.'.join(name.split('.')[:-1]) + '.png' for name in names]
#         save_paths = [os.path.join(output_folder, name) for name in names]
        
#         evalbatch(net, paths, save_paths)
...
def evaluate(net:Yolact, dataset, train_mode=False):
    ...
    if args.image is not None:
        ...
        inp, out = args.images.split('|') # CHANGED from ":" to "|"
        evalimages(net, inp, out)
        return
    elif args.video is not None:
        if ':' in args.video:
            inp, out = args.video.split('|') # CHANGED from ":" to "|"
...
if __name__ == '__main__':
    ...
    # NEW
    print("Weights:", args.trained_model)
    # END NEW
